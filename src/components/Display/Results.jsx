  ...
  return (
    <div className=" w-full h-full flex flex-col justify-center p-2 font-bold ">
      ...
      {forces.map((force, i) => {
        return (
          <div
            key={"force" + force.name + i}
            className={"w-full my-5  text-white max-sm:text-xs " + ((isPos && " animate-fade-right ") || " animate-fade-left ")}
          >
            <div className="md:hidden">{force.name}</div>

            <div className="grid grid-cols-6 h-8 md:h-14 w-full">
              {isPos &&
                    ...
                    return (
                      <>
                        <div key={"else" + force.name + j} className={" h-8 md:h-14 " + (el <= force.score && "  bg-[#0da7db] bg-opacity-90 ")}>
                          ...
                          <div
                            className={
                              " absolute text-white m-1 h-20 text-left w-40 max-sm:hidden z-50" +
                  ...
                  if (el == 0) {
                    ...
                    return (
                      <div key={"else" + force.name + j} className={" h-8 md:h-14 " + (el <= force.score && " bg-[#ffbc17] bg-opacity-90 ")}>
                        {el == 1 && <div className=" absolute text-white m-1 h-20 text-left max-sm:hidden w-52 ">{force.name}</div>}
